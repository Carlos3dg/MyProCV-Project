const langStrings = [
    ...
    {
        es: 'Proyecto inspirado en Google Calendar. Calendar App nos ayuda a organizar tareas o eventos de forma detallada, eligiendo la hora de inicio y fin de estas y la cantidad de veces que queremos que se repitan. Cuenta con la simulación de un Login para acceder a las tareas únicamente cuando esté la sesión abierta y establece enrutadores, que ayudan al usuario a saber en qué lugar de la página se encuentra o notificarle si está en un lugar no indexado (Páginas 404). Este proyecto fue realizado con React, React Router y Redux.',
        en: 'A project inspired in Google Calendar. Calendar App helps us to manage tasks or events in a detailed way, selecting the start time and end time of these and the frequency that we want to repeat them. It has a Login simulation to allow access to the tasks only when the session is open and sets routers that help the user know which part of the page is or notify him when he is on a not indexed page (404 Page). This project was built with React, React Router, and Redux.',
    },
    {
        es: 'Calendar App',
        en: 'Calendar App',
